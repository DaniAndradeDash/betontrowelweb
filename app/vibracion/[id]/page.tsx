...
import { Vibradores } from '@/types/products';
...
export default async function MachineDetailPage({ vibrador, params }: PageProps) {
    // 3. "Desenvolvemos" params con await antes de usarlo
    const { id } = await params;
    const vib = vibradores[Number(id)];
    ...
    return (
        <main className="min-h-screen bg-white text-[#171717] font-sans selection:bg-[#00AEEF] selection:text-white">
            ...
            <nav className="max-w-7xl mx-auto px-6 py-8 flex justify-between items-center">
                <Link href="/vibracion" className="flex items-center gap-2 text-sm font-medium hover:text-[#00AEEF] transition-colors group">
            ...
            <section className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 pb-20">
                ...
                <div className="space-y-6">
                    <div className="relative aspect-square rounded-[2.5rem] overflow-hidden bg-[#F5F5F5] border border-gray-100">
                        <Image src={vib.img} alt={vib.name} fill className="object-cover" priority />
                    </div>
                    <div className="p-10 bg-[#F5F5F5] rounded-[2rem] border border-gray-100">
                        ...
                        <p className="text-sm leading-relaxed text-gray-600 font-light">{vib.usage}</p>
                    </div>
                </div>
                ...
                <div className="flex flex-col justify-center">
                    ...
                    <h1 className="text-5xl md:text-7xl font-bold tracking-tighter mb-8 leading-[0.9] text-[#1A1A1A]">{vib.name}</h1>
                    <p className="text-xl text-gray-500 font-light mb-12 leading-relaxed max-w-lg">{vib.desc}</p>

                    <div className="grid grid-cols-2 gap-px bg-gray-200 border border-gray-200 rounded-3xl overflow-hidden mb-12 shadow-sm">
                        {vib.specs?.map((spec: any, i: number) => (
            ...
            <section className="bg-[#F5F5F5] py-24 px-6 border-t border-gray-100">
                <div className="max-w-7xl mx-auto">
                    <div className="flex justify-between items-end mb-12">
                        ...
                        <Link href="/vibracion" className="text-sm font-bold text-[#00AEEF] flex items-center gap-1 hover:underline">
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        {vibradores.map((prod) => (
                            <Link key={prod.id} href={`/vibracion/${prod.id}`} className="group bg-white rounded-[2rem] p-4 flex items-center gap-6 hover:shadow-xl transition-all border border-transparent hover:border-[#00AEEF]/20">
